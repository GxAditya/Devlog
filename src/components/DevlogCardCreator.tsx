    ...
    <GlassCard className="p-6 mb-8">
      <div className="space-y-6">
        ...
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-white mb-2">Create Your DevLog Card</h2>
          <p className="text-white/70">Share your development journey with a beautiful card</p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <motion.button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white/80 hover:text-white hover:bg-white/20 transition-all"
          ...
          <div className="flex items-center gap-3 p-3 bg-white/5 border border-white/20 rounded-lg">
            ...
            <div>
              ...
              <div className="text-white/60 text-sm capitalize">{selectedTemplate.theme} theme</div>
            </div>
          </div>
          ...
          <AnimatePresence>
            {showTemplates && (
              ...
              >
                {/* Dark Templates */}
                <div>
                  <h4 className="text-white/80 font-medium mb-3">Dark Themes</h4>
                  <div className="grid grid-cols-3 gap-3">
                    {darkTemplates.map((template) => (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                      >
                        <div className={`w-full h-16 ${template.preview} rounded mb-2 border border-white/20`} />
                ...
                {/* Light Templates */}
                <div>
                  <h4 className="text-white/80 font-medium mb-3">Light Themes</h4>
                  <div className="grid grid-cols-2 gap-3">
                    {lightTemplates.map((template) => (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                      >
                        <div className={`w-full h-16 ${template.preview} rounded mb-2 border border-gray-400 shadow-sm shadow-gray-300`} />
                        <div className="text-white text-sm font-medium">{template.name}</div>
                      </motion.button>
                    ))}
                  </div>
        ...
        <div className="relative">
          <textarea
            ...
            className="w-full h-40 bg-white/5 border border-white/20 rounded-xl p-4 text-white placeholder-white/60 text-lg leading-relaxed resize-none outline-none focus:border-white/40 focus:bg-white/10 transition-all"
            maxLength={280}
          />
          <div className="absolute bottom-3 right-3 text-sm text-white/50 bg-black/20 px-2 py-1 rounded">
            {content.length}/280
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-4">
          ...
          <motion.button
            ...
            className="px-8 py-3 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full text-white font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-3 text-lg shadow-lg shadow-purple-500/25"
